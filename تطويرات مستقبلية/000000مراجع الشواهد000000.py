...
class TextExtractor:
    def __init__(self, txt_file_path, xlsx_file_path):
        ...
        self.exclude_patterns = [re.compile(r'^".*"$'), re.compile(r'^قَالَ يَ.*'), re.compile(r'^قَالَ يُ.*'), re.compile(r'^قَالَتْ تَ.*'), re.compile(r'^قَالَتْ تُ.*')]
    ...
    def compare_texts(self, text_date_pairs):
        ...
        with open('مراجع الشواهد.txt', 'w', encoding='utf-8') as result_file:
